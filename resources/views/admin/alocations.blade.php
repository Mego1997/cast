                ...
                <div class="page-title">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-6 order-md-1 order-last">
                            <h3>Locations</h3>
                            <p class="text-subtitle text-muted">All Location Data</p>
                        </div>
                        <div class="col-12 col-md-6 order-md-2 order-first">
                            <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="index.html">Dashboard</a></li>
                                    <li class="breadcrumb-item active" aria-current="page">Location Database</li>
                        ...
                        <div class="card-body">
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    <h3 class="panel-title">All Location Data</h3>
                                </div>
                                <div class="panel-body">
                                    <div class="table-responsive">
                                        ...
                                        <table  class="table table-bordered table-striped" id="table1">
                                            ...
                                            <tbody>
                                            @foreach($Alllocations as $i)
                                                <tr>
                                                    <td>{{$i->ID}}</td>
                                                    <td>{{$i->location_nameEN}}</td>
                                                    <td>{{$i->location_o_nameEN}}</td>
                                                    <td>{{$i->AllCountries->name_en}}</td>
                                                    <td>{{$i->area}}</td>
                                                    <td>{{$i->location_type}}</td>
                                                    <td>{{$i->updated_at}}</td>
                                                    <td>{{$i->created_at}}</td>
                                                    @if($i->Status==1)
                                                        <td>
                                                    ...
                                                    <td style="align-items: center">
                                                        <span class="badge bg-warning"><a href="{{ url('edit-location/'.$i->ID) }}">Edit</a></span>
                                                        <span class="badge bg-danger"><a onclick="return confirm('Are you sure?')" href="{{"delete-location/".$i->ID}}">Delete</a></span>
